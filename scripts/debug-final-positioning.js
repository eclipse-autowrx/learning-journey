...
async function debugFinalPositioning() {
  console.log('🎯 Final Positioning - Ready for Production...\n');
  ...
  try {
    ...
    // Final positioning (Y inverted - from top measurements)
    ...
    console.log('\n📍 Final positioning:');
    console.log(`👤 User Name: (${centerX}, ${userNameY.toFixed(2)}) - 11 cm from top`);
    console.log(`📚 Path Name: (${centerX}, ${pathNameY.toFixed(2)}) - 13.2 cm from top`);
    console.log(`📅 Issue Date: (${centerX + issueDateXShift}, ${issueDateY.toFixed(2)}) - 14.25 cm from top + 1cm right shift`);
    ...
    page.drawText('USER NAME (11cm from top)', {
      x: 10,
      y: userNameY + 8,
      size: 10,
      font: regularFont,
    ...
    page.drawText('PATH NAME (13.2cm from top)', {
      x: 10,
      y: pathNameY + 8,
      size: 10,
      font: regularFont,
    ...
    page.drawText('DATE (14.25cm from top + 1cm right)', {
      x: 10,
      y: issueDateY + 8,
      size: 10,
      font: regularFont,
    ...
    const fileName = `final_positioning_${Date.now()}.pdf`;
    const filePath = path.join(testDir, fileName);
    
    fs.writeFileSync(filePath, pdfBytes);
    
    console.log(`\n✅ Final positioning test saved to: ${filePath}`);
    console.log(`📊 File size: ${pdfBytes.length} bytes`);
    console.log('\n🎨 Visual markers added:');
    console.log('   🔴 Red line: User name position (11 cm from top)');
    console.log('   🟢 Green line: Path name position (13.2 cm from top)');
    console.log('   🔵 Blue line: Issue date position (14.25 cm from top)');
    console.log('   ⚫ Gray line: Center vertical line');
    console.log('   🟣 Magenta line: Shifted center line for date (1 cm right)');
    console.log('\n🎉 Certificate positioning is now ready for production!');

  } catch (error) {
    console.error('❌ Error debugging final positioning:', error);
    throw error;
  }
}

// Run the debug
debugFinalPositioning();