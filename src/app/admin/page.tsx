...
import { FaPlus, FaTrash, FaGripVertical, FaRoute, FaEllipsisV } from 'react-icons/fa';
...
function AdminPageInner() {
  ...
  useEffect(() => {
    if (!isAuthenticated || !hasManageUsers || activeTab !== 'paths') return;
    fetchAllPaths();
  }, [isAuthenticated, hasManageUsers, activeTab]);
          ...
          <div className="border-b border-gray-200">
            <nav className="-mb-px flex space-x-8 px-6">
              ...
              <button
                ...
                className={`py-4 px-1 border-b-2 font-medium text-sm ${activeTab === 'paths'
                    ? 'border-blue-500 text-blue-600'
                    : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'
                }`}
              >
                          ...
                          <div className="bg-blue-50 border-b border-blue-200 px-6 py-3 mb-4 rounded-t-lg">
                            <div className="flex items-center justify-between">
                              ...
                              <div className="flex items-center space-x-3">
                                ...
                                <button
                                  onClick={() => {
                                    console.log('Button clicked! Setting modal state...');
                                    alert('Button clicked! This should show an alert.');
                                    setBulkActionType('state');
                                    setShowBulkActionModal(true);
                                    console.log('Modal state should be set now');
                                  }}
                                    ...
                                    <div className="flex items-center justify-end space-x-2">
                                      <select
                                        ...
                                        onChange={async (e) => {
                                          ...
                                          try {
                                            const response = await fetch(`/api/admin/paths/${path._id}`, {
                                              method: 'PUT',
                                              headers: {
                                                'Content-Type': 'application/json',
                                              },
...
function AddPathsModalContent({
  ...
  return (
    <div>
      ...
      {/* Test Modal - Always Show */}
      {true && (
        <div className="fixed inset-0 bg-red-500 bg-opacity-50 overflow-y-auto h-full w-full z-50">
          <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div className="mt-3">
              <h3 className="text-lg font-medium text-gray-900 mb-4">TEST MODAL - This should always show</h3>
              <p>If you can see this, the modal system works!</p>
              <button
                onClick={() => console.log('Test modal button clicked')}
      ...
      <div style={{position: 'fixed', top: 0, left: 0, background: 'yellow', padding: '10px', zIndex: 9999}}>
        Debug: showBulkActionModal={String(showBulkActionModal)}, bulkActionType={String(bulkActionType)}, selectedPaths={selectedPaths.length}
      </div>

      {/* Bulk Action Modal for Paths - Simplified Test */}
      {showBulkActionModal && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
          <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div className="mt-3">
