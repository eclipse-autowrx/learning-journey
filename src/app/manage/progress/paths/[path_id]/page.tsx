...
export default function PathProgressDetail() {
  const params = useParams();
  const path_id = params?.path_id as string;
  const [rows, setRows] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);

  const fetchDetail = async () => {
    try {
      const res = await fetch(`/api/admin/progress/paths/${path_id}/detail`);
  ...
  useEffect(() => { if (path_id) fetchDetail(); }, [path_id]);

  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold mb-4">Path Progress Detail</h1>
