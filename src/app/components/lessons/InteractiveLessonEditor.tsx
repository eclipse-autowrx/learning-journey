...
export default function InteractiveLessonEditor({ value, onChange }: InteractiveLessonEditorProps) {
  ...
  const lesson: Partial<InteractiveLesson> = {
    ...
    actions: [],
    ...value,
    trigger_source: 'learning', // Enforce this value
  };

  // Effect to initialize expanded state
  useEffect(() => {
    const initialState = (lesson.actions || []).reduce((acc: { [key: number]: boolean }, _, index) => {
      acc[index] = false; // Default to collapsed
      return acc;
    }, {});
    setExpandedState(initialState);
  }, [lesson.actions?.length]);


  const handleFieldChange = (field: keyof InteractiveLesson, fieldValue: any) => {
    onChange({ ...lesson, [field]: fieldValue });
  };

  const handleActionChange = (index: number, newAction: Action) => {
    const newActions = [...(lesson.actions || [])];
    newActions[index] = newAction;
    onChange({ ...lesson, actions: newActions });
  };

  const handleAddAction = () => {
    const newAction: Action = {
      name: `New Action ${(lesson.actions?.length || 0) + 1}`,
      path: "",
    ...
    const newActions = [...(lesson.actions || []), newAction];
    setExpandedState(prev => ({ ...prev, [newActions.length - 1]: true }));
    onChange({ ...lesson, actions: newActions });
  };

  const handleDeleteAction = (index: number) => {
    const newActions = (lesson.actions || []).filter((_, i) => i !== index);
    onChange({ ...lesson, actions: newActions });
  };

  const onDragEnd = (result: DropResult) => {
    if (!result.destination) return;
    const newActions = Array.from(lesson.actions || []);
  ...
  const handleJsonChange = (value: string) => {
    try {
      ...
      if (Array.isArray(parsedActions)) {
        onChange({ ...lesson, actions: parsedActions });
        setJsonError(null);
      } else {
    ...
    <div className="space-y-6">
      <div className="p-4 border border-gray-200 rounded-lg space-y-4 bg-gray-50">
        ...
        <div>
            ...
            <input type="text" value={lesson.name} onChange={(e) => handleFieldChange('name', e.target.value)} className="mt-1 block w-full border border-gray-300 rounded-md px-3 py-2 text-sm bg-white" />
        </div>
        <div>
            <label className="block text-sm font-medium text-gray-700">Description</label>
            <textarea value={lesson.description} onChange={(e) => handleFieldChange('description', e.target.value)} rows={3} className="mt-1 block w-full border border-gray-300 rounded-md px-3 py-2 text-sm bg-white" />
        </div>
        <div className="flex items-center space-x-8">
            <div className="flex items-center">
                <input type="checkbox" checked={lesson.auto_start} onChange={(e) => handleFieldChange('auto_start', e.target.checked)} className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded" />
                <label className="ml-2 block text-sm text-gray-900">Auto Start</label>
            </div>
            <div className="flex items-center">
                <input type="checkbox" checked={lesson.auto_run_next} onChange={(e) => handleFieldChange('auto_run_next', e.target.checked)} className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded" />
        ...
        {viewMode === 'ui' ? (
            ...
            <div>
                <Editor
                    height="400px"
                    language="json"
                    value={JSON.stringify(lesson.actions || [], null, 2)}
